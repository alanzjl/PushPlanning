...
class PathGenerator:
	...
	def draw(self):
		...
		def init():
			ax.set_xlim(-3, 4)
			ax.set_ylim(-5, 2)
			return object_path, agent, agent_, box, agent_overall
		...
		def update(frame):
			...
			text = plt.text(g_before[frame][0], g_before[frame][1], "G", fontsize = 15)

			ax.add_patch(arrow)

			return object_path, agent, agent_, box, agent_overall, arrow, text
